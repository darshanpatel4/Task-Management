...
export default function ApprovalsPage() {
    ...
    try {
      ...
      if (allAssigneeIds.size > 0) {
        ...
        if (profilesError) {
          ...
          // Proceed without assignee names if this fails, or handle error more strictly
        } else {
      ...
      const mappedTasks: Task[] = (tasksData || []).map(task => ({
        ...
        // assigneeName will be generated by displayAssigneeNames function
  ...
  const displayAssigneeNames = (assigneeIds?: string[] | null): string => {
    ...
    if (names.length > 30 && assigneeIds.length > 1) { // Arbitrary length to switch to count for multiple assignees
 ...
 const handleApprove = async (taskId: string, taskAssigneeIds?: string[] | null) => {
    ...
    try {
      ...
      // Simulate notification to assignee(s)
      if (taskAssigneeIds && taskAssigneeIds.length > 0) {
        taskAssigneeIds.forEach(assigneeId => {
          const assigneeName = assigneeDetailsMap[assigneeId]?.name || assigneeId;
          console.log(`SIMULATING NOTIFICATION: Task ID ${taskId} approved for user ${assigneeName} (ID: ${assigneeId}).`);
          // Here you would insert into your 'notifications' table
          // Example: await supabase.from('notifications').insert({ user_id: assigneeId, message: `Your task "${tasks.find(t=>t.id === taskId)?.title}" has been approved!`, type: 'task_approved', task_id: taskId });
        });
      }
                ...
                <TableBody>
                  {tasks.map((task) => (
                    <TableRow key={task.id}>
                      ...
                      <TableCell><span>{task.dueDate ? format(parseISO(task.dueDate), 'MMM d, yyyy') : 'N/A'}</span></TableCell> {/* This is due date, perhaps 'completed_at' is more relevant if available? */}
